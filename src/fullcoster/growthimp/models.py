...
"""
activity: Activity = ACTIVITIES[ActivityCategory['GROWTHIMP']]
        ...
        try:
            if hasattr(self, 'date_to'):
                return (f'{activity_short} record {self.id} submitted the {sub}: {self.user} used '
                        f'{self.experiment} from {self.date_from}/{self.time_from.isoformat()} to'
                        f' {self.date_to}/{self.time_to.isoformat()}')
            else:
                return (f'{activity_short} record {self.id} submitted the {sub}: {self.user} used '
                        f'{self.experiment} the {self.date_from}/{self.time_from.isoformat()} to'
                        f' {self.date_from}/{self.time_to.isoformat()}')
        except:
            return str(self)
